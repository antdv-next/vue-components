...
const Select = defineComponent<SelectProps<any, DefaultOptionType>>((props, { slots, expose }) => {
  ...
  const [internalSearchValue, setSearchValue] = useMergedState('', {
    value: computed(() => searchValue.value as any),
  })
  ...
  const [internalValue, setInternalValue] = useMergedState<any>(props.defaultValue as any, {
    value: computed(() => props.value as any),
  })
  ...
  return () => (
    <BaseSelect
      {...(props as any)}
      id={mergedId}
      prefixCls={props.prefixCls ?? 'rc-select'}
