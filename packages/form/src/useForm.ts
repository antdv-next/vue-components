...
export class FormStore {
  ...
  // ============================= Batch ============================
  private batchUpdate: BatchTask | undefined
  ...
  private getFieldEntitiesForNamePathList = (nameList?: NamePath[] | null): FlexibleFieldEntity[] => {
    if (!nameList) {
  ...
  private getFieldsValue = (
    ...
    const fieldEntities = this.getFieldEntitiesForNamePathList(
      ...
      Array.isArray(mergedNameList as any) ? mergedNameList : null,
    )
    ...
    fieldEntities.forEach((entity: FlexibleFieldEntity) => {
      ...
      if (!mergedFilterFunc) {
        ...
        const meta: Meta = ('getMeta' in entity ? entity.getMeta?.() : null) as Meta
  ...
  private getFieldError = (name: NamePath): string[] => {
    ...
    return fieldError.errors as string[]
  }
  ...
  private getFieldWarning = (name: NamePath): string[] => {
    ...
    const namePath = getNamePath(name)
    const fieldError = this.getFieldsError([namePath])[0]
    return fieldError.warnings as string[]
  }
