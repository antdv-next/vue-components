...
type FlattenRecord<T> = { record: T; indent: number; index: number }

// recursion (flat tree structure)
function flatRecord<T extends Record<string, any>>(
  record: T,
  indent: number,
  ...
  if (record && Array.isArray(record[childrenColumnName]) && expanded) {
    ...
    for (let i = 0; i < record[childrenColumnName].length; i += 1) {
      const tempArr = flatRecord(
        record[childrenColumnName][i] as T,
        indent + 1,
        childrenColumnName,
        expandedKeys,
        getRowKey,
