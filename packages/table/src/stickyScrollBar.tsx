...
import { getOffset } from '@v-c/util/dist/Dom/css';
...
export default defineComponent<StickyScrollBarProps>({
  ...
  setup(props, { emit, expose }) {
    ...
    let onMouseUpListener: { remove: () => void } | null = null;
    let onMouseMoveListener: { remove: () => void } | null = null;
    let onResizeListener: { remove: () => void } | null = null;
    let onScrollListener: { remove: () => void } | null = null;
    onMounted(() => {
      ...
      onResizeListener = addEventListener(
        window as any,
        'resize',
        onContainerScroll as any,
      );
    });
    onActivated(() => {
      nextTick(() => {
    ...
    watch(
      ...
      () => {
        ...
        onScrollListener = addEventListener(
