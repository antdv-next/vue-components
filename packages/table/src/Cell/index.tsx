...
function isValidElement(element: any) {
  ...
  return element && isVNode(element) && typeof element.type !== 'symbol';
}
...
export default defineComponent({
  ...
  setup(props, { slots }) {
    ...
    const onMouseenter = (event: MouseEvent, mergedRowSpan: number) => {
      ...
      if (record && props.rowType === 'body' && tableContext.rowHoverable !== false) {
    ...
    const getTitle = (vnodes: VNodeArrayChildren): any => {
    ...
    const hoverRef = shallowRef<HTMLElement | null>(null);
    watch([hovering, () => props.prefixCls, hoverRef], () => {
      if (!hoverRef.value) return;
      const cellDom = findDOMNode(hoverRef.value as any);
      if (!cellDom || cellDom.nodeType !== 1) return;
      if (hovering.value) {
        addClass(cellDom as Element, `${props.prefixCls}-cell-row-hover`);
      } else {
        removeClass(cellDom as Element, `${props.prefixCls}-cell-row-hover`);
      }
    });
    return () => {
      const {
      ...
      // ==================== Child Node ====================
      let cellProps: CellType | undefined;
      let childNode: any;
