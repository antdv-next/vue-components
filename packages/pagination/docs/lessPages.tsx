...
export default defineComponent(() => {
  ...
  const lessPagesChange = (page: number) => {
    console.log(page)
    lessPagesCurrent.value = page
  }
  ...
  return () => (
    <div>
      <Pagination
        current={lessPagesCurrent.value}
        total={80}
        showLessItems
        style={{ marginBottom: '2rem' }}
        {...iconsProps.value}
        onChange={lessPagesChange}
      />
      <Pagination
        showLessItems
        defaultCurrent={1}
      ...
      />
      <div>
        <button onClick={toggleCustomIcon}>
          Toggle Custom Icon
        </button>
        <span style={{ marginLeft: '1rem' }}>
          Is using icon:
          {' '}
          {String(useIcon.value)}
        </span>
      </div>
    </div>
  )
